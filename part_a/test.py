...
def test_neg_log_likelihood(data, theta, beta):
    ...
    log_cij = lambda x :  is_correct[x] * np.log(test_sigmoid(theta[user_id[x]] - beta[question_id[x]])) + (1 - is_correct[x]) * np.log(1 - test_sigmoid(theta[user_id[x]] - beta[question_id[x]]))
...
def update_theta_beta(data, lr, theta, beta):
    ...
    for i in range(len(is_correct)):
        ...
        q = question_id[i]
        # Update theta[u]
        sig = test_sigmoid(theta[u] - beta[q])
...
def test_irt(data, val_data, lr, iterations):
    """ Train IRT model.
    ...
    for i in range(iterations):
        neg_lld = test_neg_log_likelihood(data, theta=theta, beta=beta)
        train_neg_lld_lst.append(neg_lld)
        val_neg_lld = test_neg_log_likelihood(val_data, theta=theta, beta=beta)
        valid_neg_lld_lst.append(val_neg_lld)
        score = test_evaluate(data=val_data, theta=theta, beta=beta)[0]
        if (score > best_score):
            best_score = score
...
def test_evaluate(data, theta, beta):
...
def main():
    ...
    iterations = 160
    np.random.seed(299)
    theta, beta, val_acc_lst, train_neg_lld_lst, valid_neg_lld_lst = test_irt(train_data, val_data, lr, iterations)
    ...
    # Implement part (c)                                                #
    #####################################################################
    valid_acc = test_evaluate(data=val_data, theta=theta, beta=beta)[0]
    print("Final validation accuracy is {}".format(valid_acc))
    test_acc = test_evaluate(data=test_data, theta=theta, beta=beta)[0]
    ...
    for i in range(5):
        ...
        p_correct = test_sigmoid(theta_sort - beta_j)
